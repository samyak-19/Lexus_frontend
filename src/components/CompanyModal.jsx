...
export default function CompanyModal({ isOpen, onClose,  onSuccess, company, }) {
    ...
    if (company) {
        setForm({
            ...
            establishedDate: company.establishedDate
                ? company.establishedDate.split("T")[0]
                : "",
        });
    }
  ...
  try {
    ...
    if (company) {
      // Edit Company
      response = await api.put(`/company/${company.id}`, form, {
        headers: {
      ...
      response = await api.post("/company", form, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
